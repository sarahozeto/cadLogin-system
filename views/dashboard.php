...
<body class="<?php $_SESSION['usuario_id'] ?>"> <!-- Define a classe com base no perfil -->
    <div class="container">
        <h1>Bem-vindo, <?php $_SESSION['usuario_id'] ?>!</h1>
        <p>Esta é a visão do perfil <?php $_SESSION['usuario_id'] ?>.</p>
        <?php if ( $_SESSION['usuario_id'] == 'admin'): ?>
            <!-- Admin pode gerenciar usuários (editar e excluir) -->
            <a href="index.php?action=list" class="btn">Gerenciar Usuários (Admin)</a>
 
            <?php elseif ( $_SESSION['usuario_id'] == 'gestor'): ?>
